...
![1615098869302](img\String的intern方法.png)
...
![1621933737430](img\++ 操作数栈和常量池.png)
...
https://mp.weixin.qq.com/s/9x57vIlfPVz6CYjf9T1C2w
...
![image-20210912184042320](img\FlatMap扁平化流过程.png)
...
- toMap()  Duplicate key
  ...
  Duplicate key
  --- stream流Collectors.toMap使用时，如果重复key(比如多个null)，就会出现。
  --- 解决方案可以groupingby之类的保证key唯一，
  或者使用三参的toMap(Function<? super T, ? extends K> keyMapper,
  Function<? super T, ? extends U> valueMapper,
  BinaryOperator `<U>` mergeFunction)  ， 当key重复使用mergeFunction策略
- sort

多字段排序处理

// Num 倒序再倒序，相当于正序，sort一次倒序相当于倒序
        // list.sort(Comparator.comparing(O::getNum).reversed().thenComparing(O::getSort).reversed());
        // Num 一次倒序相当于倒序，sort不受影响
        list.sort(Comparator.comparing(O::getNum).reversed().thenComparing(O::getSort));
...
public class ThreadLocal<T> {
    // Get动作实际是往当前线程的ThreadLocalMap内部的Entry读取数据
	public T get() {
    ...
    // Set动作实际是往当前线程的ThreadLocalMap内部的Entry写入数据
    public void set(T value) {
        ...
        if (map != null)
            ...
            // Thread中的threadLocals是懒加载，set的时候才创建对象
            createMap(t, value);
    }
  
  
    void createMap(Thread t, T firstValue) {
        // key为当前ThreadLocal<T>对象
        t.threadLocals = new ThreadLocalMap(this, firstValue);
    }
  
    // 🔺 remove动作实际是ThreadLocalMap执行了remove
    public void remove() {
    ...
    static class ThreadLocalMap {
        ...
        static class Entry extends WeakReference<ThreadLocal<?>> {
            ...
            // 这个方法是父类的方法，remove的本质是Entry.clear();
            public void clear() {
                // referent即Entry构造方法的k
                this.referent = null;
            }
...
### 12.2 ThreadLocal的内存泄露

https://blog.csdn.net/puppylpg/article/details/80433271

## 13. BigDecimal

### 13.1 Non-terminating decimal expansion

BigDecimal 无限小数会抛出异常 比如 1.divide(3)  无尽小数
Non-terminating decimal expansion; no exact representable decimal result.

## 14. 泛型

### 14.1 集合泛型 extends super

[https://www.jianshu.com/p/7150fbee6def](https://www.jianshu.com/p/7150fbee6def)
...
- Integer ii = null;
  System.out.println(ii instanceof Integer);  // null instanceof xxx    false

## 16. 接口幂等

比如新增操作，用户在前端进行了多次点击而造成重复请求，因此需要在一定时间内(比如1秒)判断是否重复

### 16.1 请求参数携带唯一的标识

客户端请求时，携带唯一标识，比如请求前先向服务端请求一个唯一id
...
按请求参数key排序，然后排除一些无用字段(比如时间)，然后进行md5加密，这样在固定时间内(1秒)如果有相同的md5存在则说明重复请求
