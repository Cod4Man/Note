...
- 三大特性(线程安全的保证)
  ...
  - 有序性：指令不可重排
    ...
    - 处理器在进行重新排序是必须要考虑指令之间的数据依赖性数据依赖性 
...
- 串行垃圾回收器（Serial）: 它为**单线程**环境设计并且只使用一个线程进行垃圾回收，会暂停所有的用户线程。所以不适合服务器环境

- 并行垃圾回收器（Parallel）: 多个垃圾回收线程并行工作，此时用户线程是暂停的，适用于科学计算/大数据处理等弱交互场景

- 并发垃圾回收器（CMG: ConcMarkSweep）: 用户线程和垃圾收集线程同时执行（不一定是并行，可能交替执行），不需要停顿用户线程
...
- G1收集器的设计目标是取代CMS收集器，相比CMS，G1更加突出：
  ...
  - G1的STW更可控，G1在停顿时间上添加了**预测机制**，用户可以指定期望停顿时间
