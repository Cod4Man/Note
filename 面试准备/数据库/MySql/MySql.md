...
code <> '123' 语句，如果code is null 不会被匹配到，所以语句要改成 code <> '123' or code is null。因此mysql字段尽量不要允许NULL
...
- 在数据之外，数据库系统还维护着满足特定查找算法的数据结构，这些**数据结构以某种方式引用（指向）数据(索引和数据有联系)**，这样就可以在这些数据结构上实现高级查找算法。这种数据结构，就是索引。下图就是一种可能的索引方式示例：
  ...
  ![1621674011325](img\MySQL索引数据结构简略图.png)
...
![1621681292055](img\B+Tree数据结构图.png)
...
- 为什么说 B+树比 B树更适合实际应用中操作系统的文件索引和数据库索引？
...
![1621681819781](img\聚簇索引和非聚簇索引.png)
...
- **读已提交（READ Committed）**（**Oracle**的默认隔离级别，**开始时常用**）
  ...
  `<u>`读已提交是其他事务一修改提交，当前事务即可现`</u>`
- **可重复读（RePeatable READ）** （**MySQL**的默认隔离级别）

  **事务1执行期间，禁止其他事务修改这个字段。**

  `<u>`在一个事务中，除非当前事务的修改，否则读到的信息总是不变的。(读已提交是其他事务一修改提交，当前事务即可现)`</u>`
  ...
  | SELECT * FROM test;![img](https://images2017.cnblogs.com/blog/464291/201709/464291-20170929181434465-707008140.png) |                   SELECT * FROM test;![img](https://images2017.cnblogs.com/blog/464291/201709/464291-20170929181438200-380512861.png)                   |
  |                                                      commit                                                      |                                                                                                                                                        |
  |                                                                                                                  | SELECT * FROM test;**（此时还是null，因为当前事务还未结束，但是如果此时再去插入一条[1,1]则会报错，主键冲突，因为插入是实际存在的，只是不可现）** |
  |                                                                                                                  |                             ![img](https://images2017.cnblogs.com/blog/464291/201709/464291-20170929181446825-542860738.png)                             |
  |                                                                                                                  |                                                                         COMMIT                                                                         |
  |                                                                                                                  |                                                                         BEGIN                                                                         |
  |                                                                                                                  |                                                         SELECT * FROM test**;(此时才有数据)**                                                         |
  |                                                                                                                  |                             ![img](https://images2017.cnblogs.com/blog/464291/201709/464291-20170929181455012-85444060.png)                             |
  |                                                                                                                  |                                                                         COMMIT                                                                         |
- **串行化（Serializable）**
...
row_number() over(partition by Gender order by fenshu desc)  分组排序，先用Gender 分组，然后按fenshu 排序
...
> 条件为有索引字段，则命中的行加X锁，行中的其他字段如果有索引的化，以该字段为条件的行也会被加X锁

> 条件为非唯一索引字段的RR(RePeatable Read)模式[RC(Read Commited)模式还是X锁]，除了命中行加X锁，命中区间段(非某一行，而是区间段)会被加GAP锁
...
完整版本 ： https://www.cnblogs.com/crazylqy/p/7611069.html

## 31. for update 和 lock in share mode

1. **select * for update 的使用场景** （排他）X锁

为了让自己查到的数据确保是**最新数据**，并且查到后的数据**`<u>`只允许自己来修改的时候`</u>`**，需要用到 for update 子句。

2. **select * lock in share mode 使用场景** 共享锁（S 锁)

为了确保自己查到的数据没有被其他的事务正在修改，也就是说确保查到的数据是**最新的数据**，**`<u>`并且不允许其他人来修改数据**。**但是自己不一定能够修改数据`</u>`**（比如a,b都拿了锁，a更改了数据，因为b还拿着锁，a提交不了，直到超时），因为有可能其他的事务也对这些数据 使用了 in share mode 的方式上了 S 锁。
